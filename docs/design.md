...
- [ ] `src/cli.rs::Cli` — add `verb_arg: Option<String>` as a second positional after `verb`. Update rustdoc on the existing `verb` field if it mentions "the only positional"; add a new doc line on `verb_arg`: `"Optional positional argument for verbs that accept a name (e.g. \`create-activity <name>\`). Verbs that ignore it work as before."` No `clap(name = ...)` needed — positional order suffices.
- [ ] `src/registry.rs::Verb::dispatch` — widen the function pointer from `fn(&Snapshot) -> anyhow::Result<()>` to `fn(&Snapshot, Option<&str>) -> anyhow::Result<()>`. Update the field rustdoc to name the new parameter: `"the optional positional CLI arg (e.g. the name for \`create-activity <name>\`); \`None\` for menu invocation or when the verb takes no positional"`.
- [ ] `src/registry.rs::tests::sort_by_key_preserves_intra_category_registration_order` — update the local `noop_dispatch` test fixture from `fn(_: &Snapshot) -> Result<()>` to `fn(_: &Snapshot, _: Option<&str>) -> Result<()>` so the test still compiles.
- [ ] `src/verbs/*.rs` — update all 11 existing verb `run` signatures from `fn run(snapshot: &Snapshot)` to `fn run(snapshot: &Snapshot, _arg: Option<&str>)`. The 11 modules are: `assign_workspace`, `focus_workspace_previous`, `list_activities`, `move_window_here`, `move_window_to_activity`, `move_workspace_to_activity`, `save_activity`, `switch_activity`, `switch_activity_previous`, `switch_workspace`, `toggle_debug_tint`. All ignore the new parameter — mechanical addition. Module rustdocs need no update (the parameter is ignored).
- [ ] `src/main.rs::run` — at the two dispatch call sites (direct dispatch in the `Some(name)` arm; menu dispatch in the `None` arm), change `(verb.dispatch)(&snapshot)` to `(verb.dispatch)(&snapshot, args.verb_arg.as_deref())`. The menu arm passes `args.verb_arg.as_deref()` even though menu invocation conventionally has no positional — clap permits `jiji-do verb_arg_only` only via the `verb` slot, so menu mode always sees `verb_arg = None` (no behavioral change for the menu path).
- [ ] `src/menu.rs::prompt_name` — new helper, ~30 lines, near-clone of jiji-activities' `picker::single_select::prompt_name` (`repos/jiji-activities/src/picker/single_select.rs:212`). Signature: `pub fn prompt_name(prompt: &str) -> anyhow::Result<Option<String>>`. Spawns `fuzzel --dmenu --prompt <prompt>` with stdin piped and immediately dropped (sends EOF — fuzzel `--dmenu` reads the candidate list to EOF before drawing, so empty stdin = free-text prompt). Cancel-vs-failure discipline exactly matches the existing `menu::pick_one` (lines 36–46): `out.status.success()` + non-empty trimmed first line → `Ok(Some(name))`; success + empty → `Ok(None)`; exit code 1 → `Ok(None)` (user-cancel convention); any other non-zero or signal → `bail!("fuzzel failed (exit {}): {}", ...)`. Both the success-empty and exit-1-cancel cases collapse to `Ok(None)` per the DD: the caller treats both as "no name supplied" → exit 0 no-op. Rustdoc names both the cancel-vs-failure discipline and the empty-stdin/free-text contract.
- [ ] `src/verbs/create_activity.rs` — new module, ~25 lines. Signature: `pub fn run(_snapshot: &Snapshot, arg: Option<&str>) -> anyhow::Result<()>`. Logic: (a) if `arg` is `Some(name)` and non-empty after trim → shell directly to `jiji-activities create <name>` via `crate::proc::run_capture("jiji-activities", &["create", name])`. (b) Otherwise (no positional OR explicitly empty positional) → call `crate::menu::prompt_name("Activity name: ")?`; on `Some(typed)` shell to `jiji-activities create <typed>`; on `None` (cancel or empty Enter) return `Ok(())` (clean no-op, exit 0). Snapshot unused — the verb does not act on the focused context. Module rustdoc names: "freeform-name verb; direct-CLI path skips the prompt, menu path prompts via fuzzel `--dmenu` with empty stdin". Empty positional in the direct-CLI form (a trim that empties) routes to (b) so `jiji-do create-activity ""` from a script prompts rather than silently sending an empty `create` to `jiji-activities`.
- [ ] `src/menu.rs::tests` — new unit test `prompt_name_decode_outcomes` (or similar pure-function tests if `prompt_name` factors out a `decode(success, stdout) -> Option<String>` helper as `single_select.rs::decode_name_outcome` does — implementer's judgment whether to factor; the four-case table is small enough to inline). Pin: (success, "name\n") → `Some("name")`; (success, "  \n") → `None`; (success, "") → `None`; (false, anything) → returns the bail branch (handled at integration-test layer if not factored). If kept inline in `prompt_name`, this test is omitted and coverage relies on the shim layer.
- [ ] `src/registry.rs::REGISTRY` — append 12th `Verb` entry for `create-activity` after `list-activities`: name `"create-activity"`, label `"Create activity"`, category `Category::Activity`, `menu_visible: true`, requires `NIRI_SOCKET | FUZZEL | FORK | NIRI_ACTIVITIES` (per the ratification box — FUZZEL required even for direct-CLI form to stay simple; the conditional-FUZZEL nuance is parked in Appendix C per the architect note below), dispatch `verbs::create_activity::run`.
- [ ] `src/verbs/mod.rs` — add `pub mod create_activity;` in alphabetical position (after `assign_workspace`, before `focus_workspace_previous`).
- [ ] `src/registry.rs::tests::category_grouped_ordering_pins_workspace_before_mode_regardless_of_registration_order` — extend the trailing `assert_eq!(names, vec![...])` to include `"create-activity"` as the new last Activity-category entry (after `"list-activities"`).
- [ ] `src/registry.rs::tests::enabled_with_full_activities_capabilities_includes_all_passthrough_verbs` — add `assert!(names.contains(&"create-activity"))`.
- [ ] `tests/shims.rs::create_activity_direct_cli_skips_prompt_and_passes_name` — new positive shim test: pass `--arg` positional `jiji-do create-activity foo`; the `jiji-activities` shim records argv; the `fuzzel` shim is set to fail on invocation (`exit 99; echo "fuzzel should not be called" >&2`) so a regression where direct-CLI accidentally calls the prompt is caught. Assert recorded argv contains `"create foo"`; assert no fuzzel-was-called marker.
- [ ] `tests/shims.rs::create_activity_menu_path_prompts_and_passes_typed_name` — new positive shim test: invoke `jiji-do create-activity` with no positional; the `fuzzel` shim echoes a name (`echo 'newact'`); assert recorded `jiji-activities` argv contains `"create newact"`.
- [ ] `tests/shims.rs::create_activity_cancel_exits_zero_no_dispatch` — new negative shim test: invoke `jiji-do create-activity` (no positional); `fuzzel` shim exits 1 (cancel); assert exit 0 AND no `create` argv recorded for `jiji-activities` (only the `--version` capability-probe line per the established argv-probe-accounting pattern from `move_window_to_activity_bails_when_no_focused_window`).
- [ ] `tests/shims.rs::create_activity_empty_prompt_exits_zero_no_dispatch` — new negative shim test: invoke `jiji-do create-activity` (no positional); `fuzzel` shim exits 0 with empty stdout (user pressed Enter without typing); assert exit 0 AND no `create` argv recorded.
- [ ] `tests/shims.rs::create_activity_fuzzel_failure_propagates_nonzero` — new shim test pinning cancel-vs-failure discipline (parallel to `switch_workspace_fuzzel_failure_exits_nonzero`): invoke `jiji-do create-activity` (no positional); `fuzzel` shim prints to stderr and exits 2; assert exit non-zero (NOT 69, NOT 0), stderr contains `"fuzzel failed"`.
- [ ] `tests/shims.rs::debug_reports_filtering_upstream` — extend with `.stdout(predicates::str::contains("create-activity: filtered"))`.
