//! Native verb: insert a new workspace below the current one and focus it.
//!
//! Dispatches `niri msg action add-workspace-down` (zero arguments; the
...
pub fn run(_snapshot: &Snapshot, _args: &VerbArgs) -> anyhow::Result<()> {
    crate::niri::run_action("add-workspace-down")
}
