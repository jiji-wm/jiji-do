//! Native verb: move the focused window to a new workspace below the current one.
//!
//! Dispatches `niri msg action move-window-to-new-workspace-down`, optionally
...
pub fn run(_snapshot: &Snapshot, args: &VerbArgs) -> anyhow::Result<()> {
    niri::move_window_to_new_workspace_down(args.focus)
}
